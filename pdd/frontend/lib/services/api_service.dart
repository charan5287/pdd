...
class ApiService {
  // Active dynamic backend URL (defaults to Render Cloud Backend)
  static String _currentBaseUrl = 'https://medinow-api.onrender.com';
  ...
  static const String _geminiApiKey = String.fromEnvironment(
    'GEMINI_API_KEY',
    defaultValue: 'AIzaSyA_IrTu5AxaSZg8AemToKHp3pPYra3_7ac',
  );
    ...
    try {
      ...
      return snapshot.docs.map((doc) {
        ...
        return {
          ...
          'items': data['items'] ?? [],
          'partner': 'MediConnect Delivery Expert',
  ...
  static Future<List<dynamic>> getRefillAlerts(int userId) async {
    return await CloudService.getRefillAlerts();
  }

  static Future<List<dynamic>> getExpiryAlerts(int userId) async {
    return await CloudService.getExpiryAlerts();
  }

  static Future<List<dynamic>> getUserInventory(int userId) async {
    return await CloudService.getInventory();
  }
  ...
  static Future<void> addToInventory({
    ...
    await CloudService.addMedicine(
  ...
  static Future<Map<String, dynamic>> takeMedicine(
    ...
    await CloudService.logDose(medicineName: medicineName);
    return {"message": "Dose recorded"};
  }
  ...
  static Future<Map<String, dynamic>> getAdherence(int userId) async {
    return await CloudService.getAdherence();
  }

  // ── Reminders ────────────────────────────────────────────────────────────
  static Future<List<dynamic>> getReminders(int userId) async {
    return await CloudService.getReminders();
  }

  static Future<void> saveReminder(Map<String, dynamic> data) async {
    await CloudService.saveReminder(data);
  }

  static Future<void> toggleReminder(int reminderId) async {
    await CloudService.toggleReminder(reminderId.toString());
  }

  static Future<void> deleteReminder(int reminderId) async {
    // In our new CloudService, we use String IDs for Firestore
    await CloudService.deleteReminder(reminderId.toString());
  }
  ...
  static Future<List<dynamic>> getPrescriptionHistory(int userId) async {
    return await CloudService.getPrescriptionHistory();
  }
