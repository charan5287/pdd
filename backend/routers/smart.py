...
from services.realtime_db_service import sync_user_data_to_realtime_db
...
async def _generate_insights(logs: list, score: float, db: Session, user_id: int) -> list:
    ...
    try:
        ...
        text_resp = await generate_gemini_content(prompt)
...
def toggle_reminder(reminder_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    sync_user_data_to_realtime_db(reminder.user_id, db)
