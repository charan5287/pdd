...
class Prescription(Base):
    ...
    verification_status = Column(String, default="pending_review")  # 'pending_review', 'verified', 'flagged'
    verified_by_pharmacy_id = Column(Integer, ForeignKey("pharmacies.id"), nullable=True)
    pharmacist_notes = Column(Text, nullable=True)
    drug_interactions = Column(Text, nullable=True)  # JSON list of interaction/validation alerts
    matched_pharmacy_id = Column(Integer, ForeignKey("pharmacies.id"), nullable=True)
    created_at = Column(DateTime, default=datetime.datetime.utcnow)
