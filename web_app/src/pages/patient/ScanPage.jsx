      ...
      <div style={{ padding: '20px 20px' }}>
        ...
        {result && (
          <div className="fade-in">
            {/* Status & Pharmacist Verification Badge */}
            <div style={{
              ...
              <span className="badge" style={{
                background: '#FEF3C7', color: '#B45309', padding: '6px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700
              }}>
                ⏳ Pharmacist Review Pending
              </span>
            </div>

            {/* Drug-Drug Interaction & Clinical Safety Warnings */}
            {result.drug_interactions?.length > 0 && (
              <div style={{ background: 'white', borderRadius: 20, padding: 18, marginBottom: 16, boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: '#1A1A2E', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                  🛡️ Clinical Safety & Interaction Analysis
                </div>
                {result.drug_interactions.map((alert, idx) => {
                  const isSafe = alert.severity === 'Safe'
                  const isHigh = alert.severity === 'High'
                  const bgColor = isSafe ? '#ECFDF5' : isHigh ? '#FEF2F2' : '#FFFBEB'
                  const borderColor = isSafe ? '#10B981' : isHigh ? '#EF4444' : '#F59E0B'
                  const textColor = isSafe ? '#065F46' : isHigh ? '#991B1B' : '#92400E'
                  return (
                    <div key={idx} style={{
                      background: bgColor, borderLeft: `4px solid ${borderColor}`,
                      borderRadius: 12, padding: 12, marginBottom: 8
                    }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: textColor, marginBottom: 4 }}>
                        {alert.title}
                      </div>
